...
class Cell(val left: IntArray, val right: IntArray,
          ...
          val banner: String, val bannerHot: Boolean)
...
object Director {
    ...
    fun build(dex: Dex, leftSp: String, rightSp: String, seed: Long): Reel {
        ...
        fun push(l: IntArray, r: IntArray, banner: String, hot: Boolean) {
            cells.add(Cell(l, r, (hpL / maxL).coerceIn(0f, 1f), (hpR / maxR).coerceIn(0f, 1f), banner, hot)); gt++
        }
        ...
        for (ev in events) when (ev) {
            ...
            is Ev.Win -> repeat(16) { compose(ev.side, idle(ev.species, gt).px.copyOf(), "WIN", true) }
        }
